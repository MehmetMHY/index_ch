...
- Type `/time` or `/t` to fzf-pick a rolling time window (past 1 day, week, month, year, or all time). Set it directly with `/time 1d`, `/time 1w`, `/time 1m`, `/time 1y`, or `/time all` to clear it. The filter persists across queries (shown in the prompt as `query [1w]>`) and scopes every search to chats within that window, based on the timestamp in each chat's filename.
