...
- Type `/dump` or `/d` to fuzzy-pick (multi-select) one or more of the latest results and merge their messages into a single ch-resumable chat log at `~/Downloads/index_ch_dump_<chat_count>_<epoch>.json`. Chats are ordered oldest to newest, each chat's messages stay together in order, and every message is tagged with which original file it came from. Pass numbers to skip the picker, e.g. `/dump 1 3 5`. Unreadable files are skipped with a warning.
